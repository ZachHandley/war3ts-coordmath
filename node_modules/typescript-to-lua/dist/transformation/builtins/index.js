...
function transformBuiltinPropertyAccessExpression(context, node) {
    ...
    if (typescript_1.isStringType(context, type)) {
        return string_1.transformStringProperty(context, node);
    }
    else if (typescript_1.isArrayType(context, type)) {
        const arrayPropertyAccess = array_1.transformArrayProperty(context, node);
    ...
    if (ts.isIdentifier(node.expression)) {
        ...
        if (typescript_1.isStandardLibraryType(context, ownerType, "Math")) {
            return math_1.transformMathProperty(node);
        }
        else if (typescript_1.isStandardLibraryType(context, ownerType, "Symbol")) {
            // Pull in Symbol lib
