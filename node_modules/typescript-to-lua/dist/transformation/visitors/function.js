...
function transformFunctionLikeDeclaration(node, context) {
    ...
    if (node.body === undefined) {
        throw errors_1.UnsupportedFunctionWithoutBody(node);
    }
    let body;
    ...
    const functionExpression = lua.createFunctionExpression(lua.createBlock(transformedBody), paramNames, dotsLiteral, spreadIdentifier, flags, node);
    ...
    if (ts.isFunctionExpression(node) && node.name && scope.referencedSymbols) {
        ...
        if (symbol) {
            const symbolId = symbols_1.getSymbolIdOfSymbol(context, symbol);
...
exports.transformFunctionDeclaration = (node, context) => {
    ...
    const functionExpression = lua.createFunctionExpression(block, params, dotsLiteral, restParamName, lua.FunctionExpressionFlags.Declaration);
    const name = node.name ? identifier_1.transformIdentifier(context, node.name) : undefined;
    const isDefaultExport = export_1.hasDefaultExportModifier(node);
    if (isDefaultExport) {
        ...
        throw errors_1.MissingFunctionName(node);
    }
    ...
    if (!context.options.noHoisting && name.symbolId !== undefined) {
        ...
        const functionInfo = { referencedSymbols: (_a = functionScope.referencedSymbols, (_a !== null && _a !== void 0 ? _a : new Map())) };
