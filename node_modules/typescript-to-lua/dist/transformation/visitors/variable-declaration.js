...
function transformArrayBindingElement(context, name) {
    if (ts.isOmittedExpression(name)) {
        ...
        throw errors_1.UnsupportedKind("array binding expression", name.kind, name);
    }
}
...
function transformBindingPattern(context, pattern, table, propertyAccessStack = []) {
    ...
    for (const [index, element] of pattern.elements.entries()) {
        ...
        const propertyName = literal_1.transformPropertyName(context, (_a = element.propertyName, (_a !== null && _a !== void 0 ? _a : element.name)));
        let expression;
        if (element.dotDotDotToken) {
            ...
            if (isObjectBindingPattern) {
                ...
                const usedProperties = elements.map(e => {
                    var _a, _b;
                    return lua.createTableFieldExpression(lua.createBooleanLiteral(true), lua.createStringLiteral((_a = e.propertyName, (_a !== null && _a !== void 0 ? _a : e.name)).text, (_b = e.propertyName, (_b !== null && _b !== void 0 ? _b : e.name))));
                });
...
function checkVariableDeclarationList(node) {
    if ((node.flags & (ts.NodeFlags.Let | ts.NodeFlags.Const)) === 0) {
        throw errors_1.UnsupportedVarDeclaration(node);
    }
}
