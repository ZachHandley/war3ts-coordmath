...
function getImportPath(fileName, relativePath, node, options) {
    ...
    if (absoluteImportPath.includes(absoluteRootDirPath)) {
        ...
        throw errors_1.UnresolvableRequirePath(node, `Cannot create require path. Module does not exist within --rootDir`, relativePath);
    }
}
...
function createModuleRequire(context, moduleSpecifier, tsOriginal = moduleSpecifier) {
    ...
    if (ts.isStringLiteral(moduleSpecifier)) {
        const modulePath = shouldResolveModulePath(context, moduleSpecifier)
            ? getImportPath(context.sourceFile.fileName, moduleSpecifier.text.replace(/"/g, ""), moduleSpecifier, context.options)
