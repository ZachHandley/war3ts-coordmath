...
lhs, rhs, replacementOperator, isPostfix) {
    ...
    if (objExpression && indexExpression) {
        ...
        if (isPostfix) {
            ...
            const operatorExpression = binary_expression_1.transformBinaryOperation(context, tmp, right, replacementOperator, expression);
            ...
            const operatorExpression = binary_expression_1.transformBinaryOperation(context, accessExpression, right, replacementOperator, expression);
        ...
        const operatorExpression = binary_expression_1.transformBinaryOperation(context, tmpIdentifier, right, replacementOperator, expression);
        ...
        // return ____tmp
        const tmpIdentifier = lua.createIdentifier("____tmp");
        const operatorExpression = binary_expression_1.transformBinaryOperation(context, left, right, replacementOperator, expression);
...
function transformCompoundAssignmentStatement(context, node, lhs, rhs, replacementOperator) {
    const left = utils_1.cast(context.transformExpression(lhs), lua.isAssignmentLeftHandSideExpression);
    const right = context.transformExpression(rhs);
    const [objExpression, indexExpression] = parseAccessExpressionWithEvaluationEffects(context, lhs);
    if (objExpression && indexExpression) {
        ...
        const objAndIndexDeclaration = lua.createVariableDeclarationStatement([obj, index], [context.transformExpression(objExpression), context.transformExpression(indexExpression)]);
        const accessExpression = lua.createTableIndexExpression(obj, index);
        const operatorExpression = binary_expression_1.transformBinaryOperation(context, accessExpression, right, replacementOperator, node);
        ...
        const operatorExpression = binary_expression_1.transformBinaryOperation(context, left, right, replacementOperator, node);
        const assignmentStatements = assignments_1.transformAssignment(context, lhs, operatorExpression);
